...
function Login1(props) {  
    const [employee, setemployee] = useState({ Admin_Email: '', Password: ''});  
    const apiUrl = "https://localhost:44301/api/Admin/Login";    
    const Login = (e) => {    
            e.preventDefault();     
            const data = { Admin_Email:employee.Email, Password: employee.Password };    
            axios.post(apiUrl, data)    
            .then((result) => {    
                console.log(result.data);   
                const serializedState = JSON.stringify(result.data.UserDetails);  
                var a= localStorage.setItem('AdminData', serializedState);   
