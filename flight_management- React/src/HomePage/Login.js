...
function UserLogin(props) {  
    const [employee, setemployee] = useState({ User_Email: '', Password: ''});  
    const apiUrl = "http://localhost:63745/api/User/Login";    
    const Login = (e) => {    
            e.preventDefault();    
            debugger;   
            const data = { User_Email:employee.Email, Password: employee.Password };    
            axios.post(apiUrl, data)    
            .then((result) => {    
                debugger;  
                console.log(result.data);   
                const serializedState = JSON.stringify(result.data.UserDetails);  
               var a= localStorage.setItem('userData', serializedState);   
                console.log("A:",a)  
